...
export async function GET(
  ...
  const nextCursor = listWithComments.length === limit && listWithComments[listWithComments.length - 1]
    ? encodeURIComponent(listWithComments[listWithComments.length - 1].id)
    : null;
  return NextResponse.json({ marks: listWithComments, nextCursor });
}
