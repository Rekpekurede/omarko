...
export async function POST(
  ...
  const { data: mark } = await supabase
    .from('marks')
    .select('id, user_id, title, content, category, status, support_votes, oppose_votes, dispute_count, disputes_survived, created_at, updated_at, profiles!marks_user_id_fkey(username, avatar_url)')
    .eq('id', markId)
    .single();
