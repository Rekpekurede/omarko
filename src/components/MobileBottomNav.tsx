...
function NavItem({ href, label, active, onClick, icon, badge }: NavItemProps) {
  const wrapperClasses = `tap-press relative flex min-w-[3.5rem] flex-1 flex-col items-center justify-center gap-0.5 rounded-xl px-2 py-1.5 transition duration-150 ${
    active
  ...
  const content = (
    <span className="nav-item-inner flex flex-col items-center justify-center gap-0.5">
      {iconWithBadge}
...
export function MobileBottomNav({ isSignedIn, username }: MobileBottomNavProps) {
  ...
  return (
    <>
      <nav className="fixed inset-x-0 bottom-0 z-30 px-4 pb-[max(env(safe-area-inset-bottom),1rem)] pt-2 sm:hidden">
        <div className="mobile-bottom-nav-bar mx-auto flex max-w-lg items-center justify-between gap-2 rounded-2xl border border-border bg-black/70 px-4 py-2 shadow-xl backdrop-blur-xl">
          <NavItem
            href="/"
            label="Home"
